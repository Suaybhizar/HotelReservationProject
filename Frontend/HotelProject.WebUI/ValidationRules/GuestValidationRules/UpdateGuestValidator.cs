...
namespace HotelProject.WebUI.ValidationRules.GuestValidationRules
{
    public class UpdateGuestValidator :AbstractValidator<UpdateGuestDto>
    {
        ...
        {
            RuleFor(x => x.Name).NotEmpty().WithMessage("İsim Alanaı Boş Geçilemez");
            RuleFor(x => x.Surname).NotEmpty().WithMessage("Soyisim Alanaı Boş Geçilemez");
            RuleFor(x => x.City).NotEmpty().WithMessage("Şehir Alanaı Boş Geçilemez");
